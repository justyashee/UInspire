...
const prompts = [
  'Design a modern fintech dashboard UI',
  'Create a dark SaaS landing page',
  'Generate a mobile fitness app UI',
];
const faqs = [
  {
    q: 'How does the AI UI Generator work?',
    a: 'You describe your idea in a prompt and our AI instantly generates a professional UI layout.',
  },
  {
    q: 'Can I export the generated UI code?',
    a: 'Yes, you can export clean and responsive React / Next.js code.',
  },
  {
    q: 'Is this tool suitable for beginners?',
    a: 'Absolutely. No design experience is required.',
  },
];
...
const Home = () => {
  ...
  // Faster prompt rotation
  useEffect(() => {
    const interval = setInterval(() => {
      setIndex((prev) => (prev + 1) % prompts.length);
    }, 1800); // speed increased
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#050505] via-[#0a0a1a] to-[#0f001f] flex flex-col text-white font-sans overflow-x-hidden">
      ...
      <section className="flex-1 flex flex-col justify-center items-center text-center px-6 relative py-28 md:py-40 overflow-hidden">
        {/* Glowing orbs */}
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute -top-24 -left-16 w-80 h-80 bg-purple-800 rounded-full blur-3xl opacity-40 animate-pulse"></div>
          <div className="absolute bottom-10 right-10 w-96 h-96 bg-blue-700 rounded-full blur-3xl opacity-30 animate-pulse"></div>
        </div>
        ...
        <motion.h1
          ...
          className="relative text-5xl md:text-7xl font-extrabold mb-6 z-10"
        >
          <span className="inline-block bg-[linear-gradient(90deg,#a855f7,#60a5fa,#06b6d4,#a855f7)] 
                           bg-[length:200%_auto] bg-clip-text text-transparent 
                           animate-gradientFlow-fast">
            Transform Ideas into <br /> Stunning UI
          </span>
        </motion.h1>
        ...
        >
          ...
          <div className="relative h-10 overflow-hidden">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="absolute inset-0 text-lg md:text-xl font-medium text-cyan-300"
              >
                {prompts[index]}
              </motion.div>
            </AnimatePresence>
          ...
          <Link href="/signup">
            <button className="mt-6 w-full py-4 rounded-xl bg-purple-600 hover:bg-purple-700 font-semibold text-lg shadow-lg">
              Generate UI
            </button>
          </Link>
        </motion.div>
      ...
      <section className="h-screen py-24 bg-gradient-to-b from-[#0a0a1a] via-[#0d001a] to-[#050505] text-center">
        <motion.h2
          ...
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-4xl md:text-5xl font-bold mb-16 text-purple-400"
        >
          Why Choose Our Platform?
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl mx-auto px-8">
          {[
            {
              title: 'AI-Powered Design',
              desc: 'Generate stunning layouts instantly with smart AI assistance.',
            },
            {
              title: 'Customizable Templates',
              desc: 'Fine-tune modern templates to match your vision.',
            },
            {
              title: 'Instant Code Export',
              desc: 'Export responsive React & Next.js components in seconds.',
            },
          ].map((feature, index) => (
            <motion.div
              key={index}
              whileHover={{ scale: 1.05, y: -5 }}
              transition={{ type: 'spring', stiffness: 200 }}
              className="p-10 bg-[#0f0f1f] border border-purple-700 rounded-3xl shadow-[0_0_35px_rgba(168,85,247,0.3)]"
            >
      ...
      {/* ================= TESTIMONIALS ================= */}
      <section className="h-screen py-24 bg-gradient-to-t from-black via-[#0a0a1a] to-[#0f001f] text-center">
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-4xl md:text-5xl font-bold mb-10 text-blue-400"
        >
          Loved by Creators & Developers
        </motion.h2>
        ...
        <div className="flex flex-wrap justify-center gap-8 px-6">
          {[
            ...
            '“I created a full dashboard in minutes.”',
            '“Beautiful results, zero hassle.”',
          ].map((quote, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.05 }}
              className="bg-[#101020] border border-blue-700 rounded-2xl p-6 max-w-sm shadow-lg"
            >
              <p className="text-gray-300 italic mb-3">{quote}</p>
              <span className="text-blue-400 text-sm">– User {i + 1}</span>
            </motion.div>
      ...
      {/* ================= FAQ SECTION ================= */}
      <section className="py-28 bg-gradient-to-b from-[#0a0a1a] to-black">
        ...
        <div className="max-w-4xl mx-auto px-6 space-y-6">
          {faqs.map((faq, i) => (
            <motion.div
              ...
              className="bg-[#0f0f1f] border border-blue-700 rounded-2xl p-6 shadow-md"
            >
